...
export async function sendReportFixedEmail(report: ReportCheckingEmail) {
  const email = createReportFixedEmail(report);
...
function createReportFixedEmail({ name, message }: ReportCheckingEmail) {
  const logoUrl = getEmailLogoUrl();
  const logoMarkup = `<img src="${escapeHtml(logoUrl)}" width="56" height="56" alt="SinyalKita" style="display:block;width:56px;height:56px;border:0" />`;
  const safeName = escapeHtml(name);
  const safeMessage = escapeHtml(message);
  ...
  return {
    subject: "Report gangguan sudah FIX - SinyalKita",
    text: [
      `Halo ${name},`,
      "",
      "Yeayy, report gangguan kamu sudah kami FIX dan sudah dibenarkan.",
      "",
      `Report kamu: ${message}`,
      "",
      "Silakan coba gunakan WiFi kembali. Terima kasih sudah menunggu."
    ].join("\n"),
    html: `
      <div style="margin:0;padding:0;background:#f4f7fa;font-family:Arial,Helvetica,sans-serif;color:#0b1628">
        <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent">
          Report gangguan kamu sudah FIX dan sudah dibenarkan.
        </div>
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;background:#f4f7fa">
          <tr>
            <td align="center" style="padding:32px 12px">
                    ...
                    <div style="padding:30px 28px 28px">
                      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
                        <tr>
                          ...
                          <td style="vertical-align:middle">
                            <p style="margin:0;font-size:18px;font-weight:800;line-height:1.2;color:#0b1628">SinyalKita</p>
                            <p style="margin:4px 0 0;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#2f9a6d">Report FIX</p>
                          </td>
                        </tr>
                      </table>

                      <h1 style="margin:28px 0 0;font-size:27px;line-height:1.18;color:#0b1628">Yeayy, report gangguan kamu sudah FIX</h1>
                      <p style="margin:14px 0 0;font-size:16px;line-height:1.7;color:#42526a">
                        Halo ${safeName}, gangguan yang kamu laporkan sudah kami cek dan sudah dibenarkan.
                      </p>

                      <div style="margin:24px 0 0;border:1px solid #bfe8d7;border-radius:18px;background:#edf9f3;padding:18px 20px">
                        <p style="margin:0 0 8px;font-size:12px;font-weight:800;letter-spacing:0.12em;text-transform:uppercase;color:#2f9a6d">Report yang sudah FIX</p>
                        <p style="margin:0;font-size:14px;line-height:1.7;color:#42526a">${safeMessage}</p>
                      </div>

                      <p style="margin:22px 0 0;font-size:14px;line-height:1.7;color:#63758c">
                        Silakan coba gunakan WiFi kembali. Terima kasih sudah sabar menunggu.
                      </p>
                    </div>
                  </td>
                </tr>
