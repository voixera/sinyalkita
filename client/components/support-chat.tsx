...
export function SupportChat() {
  ...
  const visibleOnDashboard = pathname === "/dashboard";

  useEffect(() => {
    if (!visibleOnDashboard) {
      setOpen(false);
    }
  }, [visibleOnDashboard]);

  if (!ready || !token || user?.role !== "CUSTOMER" || !visibleOnDashboard) {
    return null;
  }
      ...
      <AnimatePresence>
        {open ? (
          <motion.section
            ...
            className="fixed inset-x-3 bottom-24 top-20 z-40 flex flex-col overflow-hidden rounded-2xl border border-line bg-white shadow-lift sm:inset-auto sm:bottom-24 sm:right-4 sm:top-auto sm:h-[min(620px,calc(100vh-7rem))] sm:w-[calc(100vw-2rem)] sm:max-w-[380px] lg:bottom-6 lg:right-6"
          >
